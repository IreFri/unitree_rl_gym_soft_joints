...
plots_path = os.path.join(plots_dir, "robot_push_var_750.json")
video_out_path = os.path.join(plots_dir, "H1_var_push_750.mp4")
video_out_path_hq = os.path.join(plots_dir, "simulation_video_HQ.mp4")
max_duration = 15  # Stop after 30 seconds
...
def warmup(env, actor_critic, num_steps=20):
    ...
    for _ in range(num_steps):
        obs_all = env.get_observations()
        if isinstance(obs_all, tuple):
            obs = obs_all[0]  # actor obs
        else:
            obs = obs_all
        actions = actor_critic.act_inference(obs)
        result = env.step(actions.detach())
        if len(result) == 5:
            obs, *_ = result
        else:
            obs, *_ = result
...
def apply_external_force(self, force_tensor, duration=0.1):
    ...
    if not hasattr(self, 'base_body_idx'):
        self.base_body_idx = self.gym.find_actor_rigid_body_handle(env_handle, actor_handle, "pelvis")

    self.force_tensor[:] = 0.0
    self.force_tensor[self.base_body_idx, :] = force_tensor[0, :]
    ...
    self.push_start_time = self.gym.get_sim_time(self.sim)  # ✅ Set using sim time
    self.push_end_time = self.push_start_time + duration
    self.pushing = True

    print(f"🟢 Push applied at sim_time={self.push_start_time:.2f}, will end at {self.push_end_time:.2f}")
    print(f"🔧 Applying force to body {self.base_body_idx}: {self.force_tensor[self.base_body_idx].cpu().numpy()}")
...
def play(args):
    ...
    viewer_handle = getattr(env, "viewer", None)
    if viewer_handle is None:
        viewer_handle = env.gym.create_viewer(env.sim, gymapi.CameraProperties())
    env.viewer = viewer_handle  # ✅ Make sure it's assigned to env.viewer

    env.reset()
    # print("🔧 DOF Names (first 10):", env.dof_names[:10])

    env.apply_external_force = types.MethodType(apply_external_force, env)
    env.force_tensor = torch.zeros((env.num_bodies, 3), dtype=torch.float, device=env.device)
    env.pushing = False
    t0 = T0  
    force_vector = torch.zeros((1, 3), device=env.device)
    force_vector[0, 1] = FORCE  # Push from the left (positive Y)
    push_applied = False
    env.draw_arrow_until = 0.0
    push_duration = PUSH_DURATION
    arrow_visible = False  # NEW: Track whether arrow is currently drawn
    arrow_data = None
    ...
    if JOYSTICK_GUI:
        ...
        gui = JoystickGUI()
        gui.show()
        gui.raise_()  # Bring GUI to the front
        gui.activateWindow()  # Ensure it's the active window
        QApplication.processEvents()  # Ensure Qt processes the request
        time.sleep(0.5)  # Small delay to let it update
    ...
    if RECORD_FRAMES:
        ...
        cam2_props.width = 1920  
        cam2_props.height = 1080  
    ...
    while True:
        ...
        if arrow_data is not None:
            if env.push_start_time <= sim_time <= env.push_end_time:
                if not arrow_visible:
                    print(f"➡️ Drawing arrow at sim_time={sim_time:.2f}")
                    arrow_visible = True
                env.gym.add_lines(env.viewer, env.envs[0], 3, arrow_data["lines"], arrow_data["colors"])
            elif arrow_visible and sim_time > env.push_end_time:
                print(f"❌ Hiding arrow at sim_time={sim_time:.2f}")
                arrow_visible = False
                arrow_data = None  # Stop drawing after push
        ...
        # Re-apply force for every frame during the push duration
        if getattr(env, "pushing", False) and sim_time <= getattr(env, "push_end_time", 0):
            ...
            # print(f"💥 Force applied again at sim_time={sim_time:.2f}")


        env.gym.step_graphics(env.sim)

        if pending_push and (sim_time >= T0):
            env.apply_external_force(force_vector, duration=push_duration)
            print(f"🟢 Push applied at sim_time={env.push_start_time:.2f}, will end at {env.push_end_time:.2f}")
            env.push_start_time = env.gym.get_sim_time(env.sim)  # use current sim time
            env.push_end_time = env.push_start_time + push_duration
            env.pushing = True

            # Create arrow
            end_point = env.root_states[:, 0:3][0].detach().cpu().numpy()
            force_dir = force_vector[0].detach().cpu().numpy()
            force_dir_norm = force_dir / (np.linalg.norm(force_dir) + 1e-6)
            arrow_length = 0.5
            start_point = end_point - arrow_length * force_dir_norm

            orthogonal = np.array([-force_dir_norm[1], force_dir_norm[0], 0])
            arrowhead_length = 0.2
            head1 = end_point - arrowhead_length * (force_dir_norm - 0.5 * orthogonal)
            head2 = end_point - arrowhead_length * (force_dir_norm + 0.5 * orthogonal)
            ...
            pending_push = False
            arrow_visible = False  # Will be set True below when drawing
        
        # ✅ Draw force arrow only during push window
        if arrow_data is not None:
            if env.push_start_time <= sim_time <= env.push_end_time:
                if not arrow_visible:
                    print(f"➡️ Drawing arrow at sim_time={sim_time:.2f}")
                ...
                arrow_visible = False
                arrow_data = None  # Stop drawing after push
                env.gym.clear_lines(env.viewer)  # Optional: clear residual lines
        ...
        sim_time = env.gym.get_sim_time(env.sim)
        if getattr(env, "pushing", False) and sim_time > getattr(env, "push_end_time", 0):
            print(f"🛑 Ending push at sim_time={sim_time:.2f}")
            env.force_tensor[:] = 0.0
            env.gym.apply_rigid_body_force_tensors(env.sim, gymtorch.unwrap_tensor(env.force_tensor), None, gymapi.ENV_SPACE)
            env.pushing = False
            arrow_data = None  # ✅ Remove arrow when force ends
        ...
        if hasattr(env, "cfg") and getattr(env.cfg.control, "variable_stiffness", False):
            if hasattr(env, "curr_stiffness") and hasattr(env, "curr_damping"):
                # make sure curr_stiffness is a tensor
                stiff = env.curr_stiffness
                if isinstance(stiff, torch.Tensor):
                    env.stiffness_log.append(stiff.detach().cpu().clone())
                elif isinstance(stiff, (float, int)):
                    env.stiffness_log.append(torch.tensor([stiff], dtype=torch.float32))  # wrap scalar
                elif isinstance(stiff, list):
                    env.stiffness_log.append(torch.tensor(stiff, dtype=torch.float32))
                else:
                    print(f"⚠️ Unknown curr_stiffness type: {type(stiff)}, skipping...")

                damp = env.curr_damping
                if isinstance(damp, torch.Tensor):
                    env.damping_log.append(damp.detach().cpu().clone())
                elif isinstance(damp, (float, int)):
                    env.damping_log.append(torch.tensor([damp], dtype=torch.float32))
                elif isinstance(damp, list):
                    env.damping_log.append(torch.tensor(damp, dtype=torch.float32))
                else:
                    print(f"⚠️ Unknown curr_damping type: {type(damp)}, skipping...")
        ...
        env.gym.step_graphics(env.sim)
        env.gym.draw_viewer(viewer_handle, env.sim, False)
        if env.gym.query_viewer_has_closed(viewer_handle):
            break
        ...
        if env.gym.get_sim_time(env.sim) >= max_duration:
            ...
            if len(com_history) > 0:
                ...
                if hasattr(env, "stiffness_log") and len(env.stiffness_log) > 0:
                    cleaned_stiffness_log = []
                    for s in env.stiffness_log:
                        if isinstance(s, torch.Tensor):
                            cleaned_stiffness_log.append(s)
                        elif isinstance(s, (float, int)):
                            cleaned_stiffness_log.append(torch.tensor([[s]*10], dtype=torch.float32))  # expand scalar to [1,10]
                        elif isinstance(s, list):
                            cleaned_stiffness_log.append(torch.tensor(s, dtype=torch.float32).unsqueeze(0))  # [1,10]
                        else:
                            print(f"⚠️ Skipping bad element of type {type(s)}: {s}")
                    stiffness_tensor = torch.cat(cleaned_stiffness_log, dim=0)
                    
                    cleaned_damping_log = []
                    for d in env.damping_log:
                        if isinstance(d, torch.Tensor):
                            cleaned_damping_log.append(d)
                        elif isinstance(d, (float, int)):
                            cleaned_damping_log.append(torch.tensor([[d]*10], dtype=torch.float32))
                        elif isinstance(d, list):
                            cleaned_damping_log.append(torch.tensor(d, dtype=torch.float32).unsqueeze(0))
                        else:
                            print(f"⚠️ Skipping bad element of type {type(d)}: {d}")
                    damping_tensor = torch.cat(cleaned_damping_log, dim=0)
